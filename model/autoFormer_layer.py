...
class SeriesDecomp(tf.keras.layers.Layer):
    def __init__(self, kernel_size: int) -> None:
        ...
        self.moving_avg = tf.keras.layers.AveragePooling1D(pool_size=kernel_size, strides=1, padding="same")
...
class AutoCorrelation(tf.keras.layers.Layer):
    ...
    def build(self, input_shape: Tuple[Optional[int], ...]):
        self.wq = tf.keras.layers.Dense(self.d_model, name="q")
        self.wk = tf.keras.layers.Dense(self.d_model, name="k")
        self.wv = tf.keras.layers.Dense(self.d_model, name="v")
        self.drop = tf.keras.layers.Dropout(self.attention_dropout)
        self.dense = tf.keras.layers.Dense(self.d_model, name="project")
