...
from autoFormer_layer import AutoCorrelation, SeriesDecomp
# from attention_layer import FullAttention, SelfAttention
# from dense_layer import FeedForwardNetwork
...
class AutoFormer(object):
    def __init__(
        ...
        self.project = tf.keras.layers.Conv1D(1, kernel_size=3, strides=1, padding="same", use_bias=False)

        self.project1 = tf.keras.layers.Dense(predict_sequence_length, activation=None)
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, x: tf.Tensor) -> tf.Tensor:
        ...
        _type_
            _description_
        """
        output = self.autocorrelation(x, x, x)
        x, _ = self.series_decomp1(output + x)
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def build(self, input_shape: Tuple[Optional[int], ...]) -> None:
        self.conv1 = tf.keras.layers.Conv1D(self.d_model, kernel_size=3, strides=1, padding="same")
        self.project = tf.keras.layers.Conv1D(1, kernel_size=3, strides=1, padding="same")
        self.drop = tf.keras.layers.Dropout(self.drop_rate)
        self.dense1 = tf.keras.layers.Dense(input_shape[-1])
        self.conv2 = tf.keras.layers.Conv1D(input_shape[-1], kernel_size=3, strides=1, padding="same")
...
if __name__ == '__main__':
    ...
    x = tf.random.normal([128, 10, 12])
    inputs = x

    outputs = model(inputs)
    tf_model = tf.kears.Model([inputs], [outputs])
